...
/*******************************************************************************
   TODO :
    - Known bugs :
		- Write (this procedure is hanging up for the moment...)
		- Gif are not supported and not converted to PNG on the fly
	...
	loop
		...
		if(PageLinks.exists(i)) then
			...
			--for v in PageLinks(i).first..PageLinks(i).last loop 
				rect := PageLinks(i).zero || ' ' || PageLinks(i).un || ' ' || PageLinks(i).zero + PageLinks(i).deux || ' ' || PageLinks(i).un - PageLinks(i).trois;
...
begin
	...
	if (not empty(MyHeader_Proc)) then  
	   execute immediate 'begin '|| MyHeader_Proc|| 'exception when others then raise; end;';
	end if;
exception
  when others then
...
begin
	...
	if (not empty(MyFooter_Proc)) then  
	   execute immediate 'begin '|| MyFooter_Proc|| 'exception when others then raise; end;';
	end if; 
exception
  when others then
...
begin
	...
	if(myFamily is null or myFamily = '') then
		SetFont(myFamily,myStyle,mySize);
	end if; 
	-- Restore colors
...
begin
	...
	if(myfile='' or myfile is null) then
...
begin
  null;
	...
	if(ptxt != '' or ptxt is not null) then
		if(palign='R') then
...
begin
	...
	if ( not imageExists(myFile) ) then
		...
		-- "lower" function to avoid no_data_found if url contains uppercase letters and the
		--  image is used a second time.
...
begin
	...
	l := 0;
	nl := 1;
	while(i<nb and i<1000) loop
		-- Get next character
		c := substr(s, i, 1);
		b_continue := true;
		if(c = chr(10)) then
			-- Explicit line break
			Cell(myW, pH, substr(s,j,i-j), 0, 2, '', 0, plink);
			i := i + 1;
			sep := -1;
			j := i;
			l := 0;
		...
		if (b_continue) then
			...
			if( l > myWmax) then
				...
				if( sep = -1) then
					...
					Cell(myW, pH, substr(s,j,sep-j), 0, 2, '', 0, plink);
					i := sep + 1;
				end if;
				if (b_continue2) then
					sep := -1;
	...
	-- Last chunk
	if( i != j ) then
		Cell(l / 1000 * FontSize, pH, substr(s,j), 0, 0, '', 0, plink);
    end if;
exception 
  when others then
   error('write : '||sqlerrm);
...
/* TEST HELLOWORD
	...
	--Image('http://www.laclasse.com/v2/images/b2i/fonds/fd_livret_college.png');
*/
...
----------------------------------------------------------------------------------------
-- Test inclusion of big images.
----------------------------------------------------------------------------------------
procedure testMultipleImages(pdest varchar2 default 'D') is
begin
	FPDF('P','cm','A4');
	openpdf;
	AddPage();
	SetFont('Arial','B',12);
    Image('/v2/images/picto_laclassev2.png' ,1, 1, 5);
    --Image('/v2/images/picto_ajouter.gif' ,4, 1, 5);
	Image('/v2/images/b2i/fonds/fd_livret_college.png' ,4, 1, 5);
    Cell(10,15,'Hello monde!',0,1,'C');
	Output(pdest=>pdest);
end testMultipleImages;
