...
select all_dbs.db_unique_name, database_role,nvl(backup_end,backup_date) Report_date, ARCHIVELOG_BACKUPS, DATABASE_BACKUPS, status,
  ...
  from
      (select db_unique_name, backup_date
         from rco.rc_site, (  select rownum -(trunc(sysdate) - trunc(SYSDATE - nvl(&2,1)))+ trunc(sysdate) -1  backup_date
                                from dba_objects
                          where rownum< trunc(sysdate) - trunc(SYSDATE - nvl(&2,1)) +1)
        ) all_dbs,
       (select f.DB_UNIQUE_NAME
        ...
        where  a.db_key = c.db_key
          ...
          and c.end_time between   trunc(SYSDATE - nvl(&2,1))  and trunc(sysdate)
